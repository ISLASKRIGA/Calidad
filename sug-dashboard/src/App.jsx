...
import { LayoutDashboard, MessageSquareWarning, Smile, Lightbulb, Filter } from 'lucide-react';
...
export default function App() {
  ...
  const chartByStatus = useMemo(() => {
    const counts = {};
    filteredData.forEach(d => {
      const status = d['Estatus de Resolucion interna'] || 'Pendiente';
            ...
            <table>
              ...
              <tbody>
                {filteredData.slice(0, 10).map((row, i) => (
                  <tr key={i}>
                    <td style={{ fontWeight: 'bold' }}>{row['Folio SUG'] || row['Folio Interno '] || '-'}</td>
                    <td>{row['Fecha de solicitud '] || '-'}</td>
                    <td>
